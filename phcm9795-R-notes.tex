...
\tightlist
\item
  Module 10: definitions of \texttt{r} in unequal-group sample size calculations.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Define "Active" as the first level of group:}
...
\CommentTok{\# Define "Nausea" as the first level of side\_effect:}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{drug\_aggregated}\SpecialCharTok{$}\NormalTok{group }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(drug\_aggregated}\SpecialCharTok{$}\NormalTok{group, }\AttributeTok{levels=}\FunctionTok{c}\NormalTok{(}\StringTok{"Active"}\NormalTok{, }\StringTok{"Placebo"}\NormalTok{))}
\NormalTok{drug\_aggregated}\SpecialCharTok{$}\NormalTok{side\_effect }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(drug\_aggregated}\SpecialCharTok{$}\NormalTok{side\_effect, }\AttributeTok{levels=}\FunctionTok{c}\NormalTok{(}\StringTok{"Nausea"}\NormalTok{, }\StringTok{"No nausea"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{computing-an-odds-ratio-and-its-95-confidence-interval}{%
\section{Computing an odds ratio and its 95\% confidence interval}\label{computing-an-odds-ratio-and-its-95-confidence-interval}}

We can use the \texttt{contTables} function to obtain an odds ratio and its 95\% CI, by specifying \texttt{odds=TRUE}. Here we will use the summarised HPV data from Module 6.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{nausea}\SpecialCharTok{$}\NormalTok{group }\OtherTok{\textless{}{-}} \FunctionTok{relevel}\NormalTok{(nausea}\SpecialCharTok{$}\NormalTok{group, }\AttributeTok{ref=}\StringTok{"Active"}\NormalTok{)}
\NormalTok{nausea}\SpecialCharTok{$}\NormalTok{side\_effect }\OtherTok{\textless{}{-}} \FunctionTok{relevel}\NormalTok{(nausea}\SpecialCharTok{$}\NormalTok{side\_effect, }\AttributeTok{ref=}\StringTok{"Nausea"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{drug}\SpecialCharTok{$}\NormalTok{druga }\OtherTok{\textless{}{-}} \FunctionTok{relevel}\NormalTok{(drug}\SpecialCharTok{$}\NormalTok{druga, }\AttributeTok{ref=}\StringTok{"Yes"}\NormalTok{)}
\NormalTok{drug}\SpecialCharTok{$}\NormalTok{drugb }\OtherTok{\textless{}{-}} \FunctionTok{relevel}\NormalTok{(drug}\SpecialCharTok{$}\NormalTok{drugb, }\AttributeTok{ref=}\StringTok{"Yes"}\NormalTok{)}
...
\NormalTok{hospital }\OtherTok{\textless{}{-}} \FunctionTok{readRDS}\NormalTok{(}\StringTok{"data/examples/Example\_9.1.rds"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{arthritis }\OtherTok{\textless{}{-}} \FunctionTok{readRDS}\NormalTok{(}\StringTok{"data/examples/Example\_9.2.rds"}\NormalTok{)}
...
Many power and sample size procedures are available in the \texttt{epiR} package. We will also use one function from the \texttt{pwr} package.
...
We will use three functions from the \texttt{epiR} package in this module:

\begin{itemize}
\tightlist
\item
  \texttt{epi.sscompc} to estimate the sample size to compare continuous outcomes
\item
  \texttt{epi.sscohortc} to estimate the sample size to compare two independent proportions from a cohort or cross-sectional study
\item
  \texttt{epi.sscc} to estimate the sample size to compare two independent proportions from a case-control study
\end{itemize}

We will use one function from the \texttt{pwr} package:

\begin{itemize}
\tightlist
\item
  \texttt{pwr.t.test} estimate the sample size to compare means from a paired study
\end{itemize}

\hypertarget{sample-size-calculation-for-the-independent-samples-t-test}{%
\section{Sample size calculation for the independent samples t-test}\label{sample-size-calculation-for-the-independent-samples-t-test}}

To do the problem discussed in Worked Example 10.2, we use the \texttt{epi.sscompc} function:
...
The first line contains parameters that we usually specify, with the second line usually left as the defaults. We must define the expected mean in the treatment and control groups, and the standard deviation of the measure. We specify one of \texttt{n} or \texttt{power} to be the measure to estimate, by specifying the unknown value as being equal to R's missing value, \texttt{NA}.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{epi.sscompc}\NormalTok{(}\AttributeTok{treat=}\DecValTok{90}\NormalTok{, }\AttributeTok{control=}\DecValTok{95}\NormalTok{, }\AttributeTok{n=}\ConstantTok{NA}\NormalTok{, }\AttributeTok{sigma=}\DecValTok{25}\NormalTok{, }\AttributeTok{power=}\FloatTok{0.9}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\hypertarget{sample-size-calculation-for-the-paired-t-test}{%
\section{Sample size calculation for the paired t-test}\label{sample-size-calculation-for-the-paired-t-test}}
...
\tightlist
\item
  \texttt{m1}: the mean of the first paired observations
\item
  \texttt{m2}: the mean of the second paired observations
\item
  \texttt{s\_group}: the common standard deviation
\item
  \texttt{corr}: the assumed correlation between the paired observations (conservatively set to 0)
\end{itemize}

The code below then uses the \texttt{pwr.t.test} function within the \texttt{pwr} library to estimate the number of pairs. For example, to replicate Output 10.2:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1 }\OtherTok{\textless{}{-}} \DecValTok{90}
\NormalTok{m2 }\OtherTok{\textless{}{-}} \DecValTok{95}
\NormalTok{s\_group }\OtherTok{\textless{}{-}} \DecValTok{25}
\NormalTok{corr }\OtherTok{\textless{}{-}} \DecValTok{0}

\NormalTok{s\_paired }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(}\DecValTok{2} \SpecialCharTok{*}\NormalTok{ s\_group}\SpecialCharTok{\^{}}\DecValTok{2} \SpecialCharTok{{-}} \DecValTok{2}\SpecialCharTok{*}\NormalTok{corr}\SpecialCharTok{*}\NormalTok{s\_group}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}

\NormalTok{d }\OtherTok{\textless{}{-}}\NormalTok{ ((m1 }\SpecialCharTok{{-}}\NormalTok{ m2)}\SpecialCharTok{/}\NormalTok{s\_paired)}

\FunctionTok{pwr.t.test}\NormalTok{(}\AttributeTok{d=}\NormalTok{d, }\AttributeTok{power=}\FloatTok{0.9}\NormalTok{, }\AttributeTok{type=}\StringTok{"paired"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
As per the Stata calculations, we require 528 pairs of observations (noting that \textbf{sample sizes are always rounded up}).
...
To do the problem discussed in Worked Example 10.3, we use the \texttt{epi.sscohortc} function:
...
The \texttt{epiR} package does not have a function to estimate sample size and power directly for a relative risk, but we can use the \texttt{epi.sscohortc} function. To do this, we recognise that the assumed rate in the exposed group will equal the rate in the unexposed group multiplied by the relative risk.

Here we will replicate Output 10.4, where p0=0.5 and the desired relative risk to detect is 1.5. So we specify \texttt{irexp0\ =\ 0.5} and \texttt{irexp1\ =\ 1.5\ *\ 0.5}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{epi.sscohortc}\NormalTok{(}\AttributeTok{irexp1=}\FloatTok{1.5}\SpecialCharTok{*}\FloatTok{0.5}\NormalTok{, }\AttributeTok{irexp0=}\FloatTok{0.5}\NormalTok{, }\AttributeTok{n=}\ConstantTok{NA}\NormalTok{, }\AttributeTok{power=}\FloatTok{0.9}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{estimating-sample-sizes-with-unequal-groups}{%
\section{Estimating sample sizes with unequal groups}\label{estimating-sample-sizes-with-unequal-groups}}

To change the allocation ratio in any of these study type, we can specify \texttt{r}. Note that the definition of \texttt{r} differs slightly depending on the function used, so it pays to check the help-file for each function. In summary:

\begin{itemize}
\tightlist
\item
  for \texttt{epi.sscompc()}, \texttt{r} is the number in the treatment group divided by the number in the control group
\item
  for \texttt{epi.sscohortc()}, \texttt{r} is the number in the exposed group divided by the number in the unexposed group
\item
  for \texttt{epi.sscc()}, \texttt{r} is the number in the control group divided by the number in the case group
\end{itemize}
